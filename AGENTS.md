...
Web scraper for Brazilian research funding opportunities ("editais"). Collects from CAPES, CNPq, FINEP, FAPESB, SETEC — stores in SQLite, exports to CSV/Excel/HTML, and can be built as a Windows executable (GUI).
...
- **Single parser**: `python crawler/parsers/capes.py` (or `cnpq.py`, `finep.py`, `fapesb.py`, `setec.py`)
