...
class FapesbParser:
    def __init__(self, max_items: int | None = 50):
        self.url = "https://www.fapesb.ba.gov.br/category/edital/"
    ...
    async def _goto_with_retry(
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                if attempt < attempts:
                    ...
                    logger.warning(
                        "FAPESB navigation failed (attempt %s/%s): %s. Retrying in %.1fs",
                        attempt,
                        attempts,
                        exc,
                        sleep_time,
        ...
        raise RuntimeError(f"FAPESB navigation failed after {attempts} attempts") from last_exc
    ...
    async def parse(self, db, max_items: int | None = None) -> dict[str, Any]:
        ...
        async with async_playwright() as p:
            ...
            items = await page.locator(".edital-item").all()
            ...
            for item in iterable:
                try:
                    title_locator = item.locator(".edital-title h3 p a")
                    ...
                    if link and not link.startswith("http"):
                        link = f"https://www.fapesb.ba.gov.br{link}"
