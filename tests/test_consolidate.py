...
class TestNormalizeTitle:
    ...
    def test_strips_pdf_suffix(self):
        assert normalize_title("Edital nº 05/2024 - Programa X - pdf, 91kb") == (
            "edital no 05/2024 - programa x"
        )
...
class TestExtractRef:
    ...
    def test_bare_number_after_second_kind(self):
        assert extract_ref("Chamada nª 20 do Edital 14/2022") == ("edital", "14", "2022")
...
class TestConsolidate:
    def test_duplicate_titles_deduplicated(self):
        rows = [
            _row("Edital nº 07/2026 - CAPES/Cofecub, formato, pdf, 545kb", link="https://a", id=1),
            _row("Edital nº 07/2026 - CAPES/Cofecub, formato, pdf, 545kb", link="https://b", id=2),
        ]
    ...
    def test_related_docs_grouped_under_core(self):
        rows = [
            _row("Alteração do Edital nº 07/2026 - CAPES/Cofecub, formato, pdf, 48kb", pub="2026-04-01", id=2),
            _row("Edital nº 07/2026 - CAPES/Cofecub, formato, pdf, 545kb", pub="2026-02-10", id=1),
            _row("Lista de inscritos no Edital n° 7/2026 - Programa CAPES/COFECUB, formato, pdf, 90kb", pub="2026-05-01", id=3),
        ]
        groups = consolidate_editais(rows)
        assert len(groups) == 1
        g = groups[0]
    ...
    def test_number_collision_split_by_subject(self):
        rows = [
            _row("Edital nº 5/2026 - Programa CAPES - PURDUE de Projetos conjuntos, formato, pdf, 257kb", id=1),
            _row("Edital Conjunto nº 5/2026 - Programa OBEDUC-EI - Observatório da Educação Especial, formato, pdf, 80kb", id=2),
        ]
        groups = consolidate_editais(rows)
    ...
    def test_empty_subject_inherits_group(self):
        rows = [
            ...
            _row("Lista de Inscritos no Edital nº 14/2022 do Programa CAPES/Humboldt", pub="2022-03-01", id=2),
            _row("Alteração do Edital n° 14/2022 - formato, pdf, 49kb", pub="2022-02-01", id=3),
        ]
        groups = consolidate_editais(rows)
        assert len(groups) == 1
